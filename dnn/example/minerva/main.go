...
func main() {
	...
	network := training.Network{
		Layers: []layers.Layer{
			layers.NewFullyConnectedLayer(to, 784, 256),
			layers.NewReluLayer(to),
			layers.NewFullyConnectedLayer(to, 256, 100),
			layers.NewReluLayer(to),
			layers.NewFullyConnectedLayer(to, 100, 100),
			layers.NewReluLayer(to),
			layers.NewFullyConnectedLayer(to, 100, 10),
		},
	}
