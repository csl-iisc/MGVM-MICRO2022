...
func (c InterleavingConverter) ConvertExternalToInternal(external uint64) uint64 {
	...
	// fmt.Println(addr, addr%roundSize, roundSize, belongsTo, c.CurrentElementIndex)

	if belongsTo != c.CurrentElementIndex {
		fmt.Println(fmt.Sprintf("%x", roundSize), c.InterleavingSize, c.TotalNumOfElements, fmt.Sprintf("%x", addr), fmt.Sprintf("%x", addr%roundSize))
