...
var _ = Describe("BufferedSender", func() {
	...
	It("should send", func() {
		...
		buffer.EXPECT().Pop()
		sent := sender.Tick(10)
		Expect(msg1.Meta().SendTime).To(Equal(akita.VTimeInSec(10)))
		...
		sent = sender.Tick(11)
		Expect(sent).To(BeTrue())
		Expect(msg2.Meta().SendTime).To(Equal(akita.VTimeInSec(11)))
	})
	...
	It("should do nothing if send failed", func() {
		msg1 := &sampleMsg{}
		buffer.EXPECT().Peek().Return(msg1)
		port.EXPECT().Send(msg1).Return(akita.NewSendError())
