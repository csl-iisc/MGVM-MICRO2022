...
__kernel void
reduce(__global const float *g_idata, __global float *g_odata,
       __local float *sdata, const unsigned int n)
{
...
__kernel void
reduceNoLocal(__global float *g_idata, __global float *g_odata,
       unsigned int n)
{
    float sum = 0.0f;
    for (int i = 0; i < n; i++)
