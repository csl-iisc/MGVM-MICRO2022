...
type MergeSortFirstKernel struct {
	Input               driver.GPUPtr
	Result              driver.GPUPtr
	ListSize            uint32
	Padding             uint32
...
// KernelArgs defines kernel arguments
type MergeSortPassKernel struct {
	Input               driver.GPUPtr
	Result              driver.GPUPtr
	NRElems             uint32
	ThreadsPerDiv       uint32
	ConstStartAddr      uint32
	Padding             uint32
	HiddenGlobalOffsetX int64
	HiddenGlobalOffsetY int64
	HiddenGlobalOffsetZ int64
...
// KernelArgs defines kernel arguments
type MergePackKernel struct {
	Orig                driver.GPUPtr
	Result              driver.GPUPtr
	ConstStartAddr      uint32
	NullElems           uint32
	FinalStartAddr      uint32
	Padding             uint32
	HiddenGlobalOffsetX int64
	HiddenGlobalOffsetY int64
	HiddenGlobalOffsetZ int64
...
type Benchmark struct {
	...
	mergesortfirstkernel *insts.HsaCo
	mergesortpasskernel  *insts.HsaCo
	mergepackkernel      *insts.HsaCo

	ListSize     int
	Divisions    int
	OriginalList []float32
	ResultList   []float32
...
func (b *Benchmark) loadProgram() {
	...
	b.mergesortfirstkernel = kernels.LoadProgramFromMemory(hsacoBytes, "mergeSortFirst")
	if b.mergesortfirstkernel == nil {
		log.Panic("Failed to load kernel binary")
	}
	b.mergesortpasskernel = kernels.LoadProgramFromMemory(hsacoBytes, "mergeSortPass")
	if b.mergesortpasskernel == nil {
		log.Panic("Failed to load kernel binary")
	}
	b.mergepackkernel = kernels.LoadProgramFromMemory(hsacoBytes, "mergepack")
	if b.mergepackkernel == nil {
		log.Panic("Failed to load kernel binary")
	}
}
