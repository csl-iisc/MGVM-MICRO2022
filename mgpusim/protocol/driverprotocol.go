...
// A WritePageTableH2DReq is a request that asks the DMAEngine to copy the page table
// from the host to the device
type WritePageTableH2DReq struct {
	akita.MsgMeta
	SrcBuffer  []byte
	DstAddress uint64
}
...
// Meta returns the meta data associated with the message.
func (m *WritePageTableH2DReq) Meta() *akita.MsgMeta {
	return &m.MsgMeta
}

// NewWritePageTableH2DReq created a new MemCopyH2DReq
func NewWritePageTableH2DReq(
	time akita.VTimeInSec,
	src, dst akita.Port,
	srcBuffer []byte,
	dstAddress uint64,
